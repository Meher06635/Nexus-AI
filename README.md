...
Agent workflow
Receive command (voice → Whisper, or typed).
Inject memories + short chat history into the system prompt.
LangChain AgentExecutor calls ChatOllama with structured tools.
Tools perform desktop/browser automation; observations return to the model.
Final natural-language reply is persisted to SQLite (conversations + tasks).
Example commands
Say / type	Behavior
